...
class TestCategorical(object):
    ...
    def test_summary(self):
        ...
        assert np.allclose(
            summary["ci_lower"],
            np.array(
                [0.4646901340180582, 0.309544, 0.6453118311511006, 0.4020018007729973, 0.303982, 0.5912276177282552]
            ),
        )
        assert np.allclose(
            summary["ci_upper"],
            np.array(
                [0.5263901434844195, 0.498147, 0.692903881232388, 0.5979981992270027, 0.496018, 0.7421057156050781]
            ),
        )
    ...
    def test_difference(self):
        ...
        assert np.allclose(diff["difference"], np.array([0.100000, 0.091694]))
        assert np.allclose(diff["ci_lower"], np.array([-0.03719748, -0.0075254]))
    ...
    def test_multiple_difference(self):
        ...
        assert np.allclose(
            ...
            np.array([1.00000000e00, 8.38342202e-01, 1.67371350e-04, 3.74330946e-01, 1.98857657e-07]),
        )
        assert np.allclose(
            diff["p-value"], np.array([9.553332e-01, 1.67668440e-01, 3.34742700e-05, 7.48661891e-02, 3.97715314e-08])
        )
        assert np.allclose(
            diff["adjusted ci_lower"], np.array([-0.173024, -0.27489017, -0.42153065, -0.22209041, -0.39307973])
        )
        assert np.allclose(
            diff["adjusted ci_upper"], np.array([0.180717, 0.08258247, -0.10411038, 0.03870244, -0.13744367])
        )

        diff = self.test.multiple_difference("test", groupby="country", level_as_reference=False)
        assert np.allclose(
            diff["adjusted p-value"], np.array([6.208740e-01, 3.36319783e-02, 2.99464756e-01, 1.30744685e-17])
        )
        assert np.allclose(diff["p-value"], np.array([1.552185e-01, 8.40799458e-03, 7.48661891e-02, 3.26861713e-18]))
        assert np.allclose(diff["adjusted ci_lower"], np.array([-0.074839, -0.32426934, -0.03474758, -0.2232184]))
        assert np.allclose(diff["adjusted ci_upper"], np.array([0.274839, -0.00906399, 0.21813554, -0.12391703]))

    def test_differences(self):
        with pytest.raises(ValueError):
            self.test.differences(("bad_value", "bad_value"))

        diff = self.test.differences(
            [
                (("gb", "control"), ("us", "control")),
                (("gb", "test"), ("us", "control")),
                (("gb", "test2"), ("us", "control")),
                (("us", "test"), ("us", "control")),
                (("us", "test2"), ("us", "control")),
        ...
        diff = self.test.differences([("control", "test"), ("test2", "test")], groupby="country")
        assert np.allclose(
            diff["adjusted p-value"], np.array([6.208740e-01, 3.36319783e-02, 2.99464756e-01, 1.30744685e-17])
        )
        assert np.allclose(diff["p-value"], np.array([1.552185e-01, 8.40799458e-03, 7.48661891e-02, 3.26861713e-18]))
    ...
    def test_multiple_difference_level_as_reference(self):
        diff = self.test.multiple_difference("test", groupby="country", level_as_reference=True)
        assert np.allclose(diff["adjusted ci_lower"], np.array([-0.274839, 0.00906399, -0.21813554, 0.12391703]))

    def test_differences_plot(self):
        ch = self.test.differences_plot(
            [
                (("gb", "control"), ("us", "control")),
                (("gb", "test"), ("us", "control")),
                (("gb", "test2"), ("us", "control")),
    ...
    def test_achieved_power(self):
        power_df = self.test.achieved_power(
            level_1=("us", "control"), level_2=("us", "test"), mde=0.1429118527, alpha=0.05
        )
    ...
    def test_achieved_power_groupby(self):
        power_df = self.test.achieved_power(
            level_1="control", level_2="test", mde=0.1429118527, alpha=0.05, groupby="country"
        )
        power = power_df.achieved_power
        ...
        self.data = pd.DataFrame(
            {
                ...
                "success": [500, 8, 100, 510, 8, 100, 520, 9, 104, 530, 7, 100, 530, 8, 103],
                "total": [1010, 22, 150, 1000, 20, 153, 1030, 23, 154, 1000, 20, 150, 1040, 21, 155],
...
class TestOrdinalPlusTwoCategorical(object):
    ...
    def test_difference(self):
        ...
        df = self.test.difference(level_1="test", level_2="control", groupby=["country", "days_since_reg"])
        assert len(df) == 10
        assert "country" in df.columns
        assert "days_since_reg" in df.columns
    ...
    def test_multiple_difference(self):
        ...
        df = self.test.multiple_difference(
            level="control", groupby=["country", "days_since_reg"], level_as_reference=False
        )
        assert len(df) == 20

        df = self.test.multiple_difference(
            level="us", groupby=["variation_name", "days_since_reg"], level_as_reference=False
        )
        assert len(df) == 15

        df = self.test.multiple_difference(level=1, groupby=["country", "variation_name"], level_as_reference=False)
        assert len(df) == 24

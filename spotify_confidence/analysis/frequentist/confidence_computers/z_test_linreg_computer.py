...
class ZTestLinregComputer(ZTestComputer):
    ...
    def __init__(self, numerator, numerator_sumsq, denominator, ordinal_group_column, interval_size, feature_column, feature_sum_squares_column, feature_cross_sum_column, method_column):
    ...
    def _estimate_slope(self, df) -> Series:
        ...
        Xy0 = np.zeros((k+1, 1))
        Xy0[0, ] = col_sum(df[self._numerator])
        Xy0[1:(k+1), ] = np.atleast_2d(col_sum(df[self._feature_cross])).reshape(-1, 1)
    ...
    #                 [self._numerator, self._denominator, POINT_ESTIMATE, CI_LOWER, CI_UPPER, REGRESSION_PARAM]
    #         ]
    #     )